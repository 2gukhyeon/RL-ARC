...
TABC_LONG_ALIGN_PROMPT = (
    ...
    "The assistant first thinks about the reasoning process internally, provides the user with the final answer, "
    "then analyzes its confidence about the solution and then provides the user with its confidence level. "
    ...
    "1) reasoning confidence, enclosed within <reasoning_confidence> </reasoning_confidence> tags, and "
    "2) answer confidence, enclosed within <answer_confidence> </answer_confidence> tags. "
    "Each confidence score must be a number between 0 and 1 (inclusive). "
    "Here are some guidelines for the analysis: "
    "1. Your task is to point out aspects where the model's thinking might be incorrect, "
    "or where ambiguity may exist in the solution steps or in the reasoning process itself.\n"
    ...
    "4. Reasoning confidence represents how truthfully the model reasons about the given question.\n"
    "5. Answer confidence represents how accurately the model answers the question based on its reasoning process.\n"
    "6. Uncertainties may differ from errors. For example, uncertainties may arise from ambiguities in the question "
    "or from the application of a particular lemma or proof.\n"
    "7. List plausible uncertainties. Avoid generic statements and be as specific as possible when describing uncertainties.\n"
    "8. The uncertainty analysis must be enclosed within <analysis> </analysis> tags.\n"
