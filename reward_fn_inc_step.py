...
def get_step_confidence_min(content):
    """Extract step confidences inside <think> and return their minimum."""
    ...
    if not step_conf_matches:
        return None

    try:
        step_confidences = [
            float(step_conf)
            for step_conf in step_conf_matches
        ]

        for step_conf in step_confidences:
            if not 0 <= step_conf <= 1:
                return None
...
def get_step_confidence_product(content):
    """Extract step confidences inside <think> and return their product."""
    think_pattern = r"<think>(.*?)</think>"
    step_conf_pattern = r"<step_confidence>(.*?)</step_confidence>"

    think_matches = re.findall(think_pattern, content, re.DOTALL | re.MULTILINE)
    if not think_matches:
        return None

    step_conf_matches = re.findall(
        step_conf_pattern, think_matches[-1], re.DOTALL | re.MULTILINE
    )
    if not step_conf_matches:
        return None
...
def format_reward(format_pattern,completions, **kwargs):
    ...
    elif format_pattern == "tabc_step":
        pattern = r".*?</think>\s*<answer>.*?</answer>\s*<analysis>.*?</analysis>\s*\Z"
    ...
    for i, match in enumerate(matches):
        if match:
            ...
            if format_pattern == "tabc_step":
                step_confidence_product = get_step_confidence_product(content)
                # step_confidence_product = get_step_confidence_min(content) # min
                if step_confidence_product is None:
                    matches[i] = 0.0
                else:
                    matches[i] = 1.0

            elif 'c' in format_pattern:
...
def brier_reward(format_pattern,completions,answer,source=None, **kwargs):
    ...
    for content,cr,fr in zip(completion_contents,correctness_rewards,format_rewards):
        ...
        try:
            ...
            elif format_pattern == "tabc_step": # product of step confidences
                conf = get_step_confidence_product(content)
                # conf = get_step_confidence_min(content) # min
...
def mean_confidence_reward(format_pattern,completions,answer, **kwargs):
    ...
    for content,e in zip(completion_contents,eval_contents):
        ...
        if format_pattern == "tabc_align": # ours
            ...
            confidence = get_step_confidence_product(content)
            # confidence = get_step_confidence_min(content)
